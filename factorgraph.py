...
class Graph(object):
    ...
    def rv(self, name, n_opts, labels=[], pots=[], meta={}, debug=DEBUG_DEFAULT):
        '''
        ...
        Returns:
            RV
        '''
        rv = RV(name, n_opts, labels, pots, meta, debug)
        self.add_rv(rv)
        return rv
    ...
    def _sorted_nodes(self):
        ...
        facs = self._factors
        nodes = list(rvs) + facs
        return sorted(nodes, key=lambda x: x.n_edges(), reverse=True)
    ...
    def print_rv_marginals(self, rvs=None, normalize=False, printm=True, printf=True, fp=None):
        '''
        ...
        for rv, marg in tuples:
            ...
            if rv.name.rfind('_') == -1:
                pair = sorted(zip(vals, marg), key=lambda x:x[1], reverse=True)
            else:
        ...
        if printf:
            if fp == None:
                fp = 'outcome/rv_marginals.txt'
            fp = open(fp, 'w', encoding='utf-8')
            json.dump(msgj, fp, ensure_ascii=False)
...
class RV(object):
    ...
    def __init__(self, name, n_opts, labels=[], pots=[], meta={}, debug=DEBUG_DEFAULT):
        '''
        Args:
            name (str)                must be globally unique w.r.t. other RVs
            n_opts (int)              how many values it can take
...
class Factor(object):
    ...
    def init_lbp(self):
        ...
        #self._outgoing = self._potential
    ...
    def recompute_outgoing(self, normalize=False):
        ...
        for i, rv in enumerate(self._rvs):
            ...
            if self.debug:
                assert self._outgoing[i].shape == (rv.n_opts, ), '%r, %r' % (self._outgoing[i].shape, (rv.n_opts, ))
            if normalize:
                o = divide_safezero(o, sum(o))
            self._outgoing[i] = o
            convg = convg and \
